        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <form action="{{ route('course.update', $detail->id) }}" method="POST" autocomplete="off" enctype="multipart/form-data"> @csrf @method('PUT')
                        <div class="card-body">
                            <div class="row gy-3">
                                <div class="form-group">
                                    ...
                                    <select name="course_category_id" class="form-control select2 @error('course_category_id') is-invalid @enderror">
                                        ...
                                        @foreach ($category as $item)
                                                <option value="{{ $item->id }}" @if($detail->course_category_id === $item->id) selected @endif>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" class="form-control  @error('title') is-invalid @enderror" placeholder="Masukkan judul" value="{{ old('title', $detail->title) }}">
                                    @error('title')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="mentor_id" class="form-control select2 @error('mentor_id') is-invalid @enderror">
                                        ...
                                        @foreach ($mentor as $item)
                                                <option value="{{ $item->id }}" @if($detail->mentor_id === $item->id) selected @endif>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('mentor_id')
                                        <span class="invalid-feedback" role="alert">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="job" class="form-control  @error('job') is-invalid @enderror" placeholder="Masukkan pekerjaan" value="{{ old('job', $detail->job) }}">
                                    @error('job')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="time" class="form-control  @error('time') is-invalid @enderror" placeholder="Masukkan waktu" value="{{ old('time', $detail->time) }}">
                                    @error('time')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                ...
                                <div class="form-group">
                                    ...
                                    @enderror
                                    <div class="mt-3">
                                        <img id="imagePreview" src="{{ $detail->image ? asset($detail->image) : '' }}" alt="Preview Gambar" style="max-width: 300px; display: none;" />
                                    </div>
                                </div>
                            </div>
                        </div>
